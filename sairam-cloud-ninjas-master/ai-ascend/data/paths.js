export const PATHS = [
  {
    id: "devops",
    title: "DevOps Engineer",
    role: "Platform",
    level: "Beginner",
    durationHours: 24,
    description: "Core DevOps roadmap with main topics from Linux to Terraform and projects.",
    tags: ["ci-cd", "automation", "infra", "linux", "aws", "docker", "kubernetes", "terraform"],
    videos: [
      {
        id: "devops-topic-1",
        title: "Linux and Bash",
        durationMin: 120,
        url: "https://youtube.com/playlist?list=PLdpzxOOAlwvIBIRWcReRV-m2kgIW6V6gr",
        tags: ["linux", "bash"]
      },
      {
        id: "devops-topic-2",
        title: "SDLC",
        durationMin: 90,
        url: "https://www.youtube.com/watch?v=RkIK5t0Jbn4",
        tags: ["sdlc"]
      },
      {
        id: "devops-topic-3",
        title: "AWS Services",
        durationMin: 140,
        url: "https://youtu.be/Ae3xyW2Ef0c?si=vprO-mMcroDfcPYP",
        tags: ["aws", "cloud"]
      },
      {
        id: "devops-topic-4",
        title: "Docker",
        durationMin: 110,
        url: "https://youtu.be/RqTEHSBrYFw?si=hjb4_rhLxrAVLMa8",
        tags: ["docker", "containers"]
      },
      {
        id: "devops-topic-5",
        title: "Kubernetes",
        durationMin: 120,
        url: "https://youtu.be/6_gMoe7Ik8k?si=c3CagvC6fT3DAV-k",
        tags: ["kubernetes", "k8s"]
      },
      {
        id: "devops-topic-6",
        title: "CI/CD with GitHub Actions",
        durationMin: 100,
        url: "https://youtu.be/icZUzgtz_d8?si=EI201zwhOA9oXaS7",
        tags: ["cicd", "github-actions"]
      },
      {
        id: "devops-topic-7",
        title: "Terraform",
        durationMin: 110,
        url: "https://youtu.be/xUtGqC-NXJE?si=RJL3wW1uSJk-XQ5p",
        tags: ["terraform", "iac"]
      },
      {
        id: "devops-topic-8",
        title: "Projects",
        durationMin: 150,
        url: "https://youtu.be/e42hIYkvxoQ?si=HCXYj0Hq7LKbDk-l",
        tags: ["projects", "capstone"]
      }
    ]
  },
  {
    id: "frontend",
    title: "Backend Developer",
    role: "Web UI",
    level: "Beginner",
    durationHours: 14,
    description: "Server-side architecture, API development, databases, and production backend practices.",
    tags: ["ui", "javascript", "css"],
    videos: [
      {
        id: "frontend-topic-1",
        title: "Frontend",
        durationMin: 90,
        url: "https://youtu.be/7dSJubxFWv0?si=mH2JxxqaAklAJjeH",
        tags: ["frontend", "basics"]
      },
      {
        id: "frontend-topic-2",
        title: "What is Backend",
        durationMin: 90,
        url: "https://youtu.be/XBu54nfzxAQ?si=4C2Ckmy8-jYC23Im",
        tags: ["backend", "basics"]
      },
      {
        id: "frontend-topic-3",
        title: "Networking Basics",
        durationMin: 90,
        url: "https://youtu.be/IPvYjXCsTg8?si=FCYPT5BNkeC3QKOg",
        tags: ["networking", "basics"]
      },
      {
        id: "frontend-topic-4",
        title: "Client-Server Architecture",
        durationMin: 90,
        url: "https://youtu.be/yioOQ4ItYuo?si=3P-IDOtq48aSARo5",
        tags: ["client-server", "architecture"]
      },
      {
        id: "frontend-topic-5",
        title: "Monolith vs Microservices Architecture",
        durationMin: 90,
        url: "https://youtu.be/NdeTGlZ__Do?si=1nLhabVxkwkFcmJV",
        tags: ["architecture", "microservices", "monolith"]
      },
      {
        id: "frontend-topic-6",
        title: "How to Choose a Backend Framework",
        durationMin: 90,
        url: "https://youtu.be/nVPkwGqhfQI?si=GuRioZElvzNQ_rbi",
        tags: ["backend", "frameworks"]
      },
      {
        id: "frontend-topic-7",
        title: "How to Choose Database for a Project",
        durationMin: 90,
        url: "https://youtu.be/kkeFE6iRfMM?si=bpOsTP-gYiBiIdaW",
        tags: ["database", "project"]
      },
      {
        id: "frontend-topic-8",
        title: "Spring Framework",
        durationMin: 90,
        url: "https://youtu.be/-Fe0zk-F4OA?si=6TTr1E6kpDPcHyYn",
        tags: ["spring", "framework"]
      }
    ]
  },
  {
    id: "backend",
    title: "Frontend Developer",
    role: "Web UI",
    level: "Beginner",
    durationHours: 36,
    description: "Responsive UI design, JavaScript fundamentals, and modern frontend practices.",
    tags: ["frontend", "react", "nextjs", "javascript", "tailwind", "ai-tools", "git"],
    videos: [
      {
        id: "frontend-2026-1",
        title: "HTML & CSS Full Course (2026 Edition)",
        durationMin: 180,
        url: "https://www.youtube.com/watch?v=XHYm2NsDJ4k",
        tags: ["html", "css", "responsive", "semantic-html", "flexbox", "grid"]
      },
      {
        id: "frontend-2026-2",
        title: "Tailwind CSS v4 & Modern CSS (Hindi)",
        durationMin: 150,
        url: "https://www.youtube.com/watch?v=WRwHc8u2W8w",
        tags: ["tailwind", "css", "hindi", "utility-first"]
      },
      {
        id: "frontend-2026-3",
        title: "JavaScript Full Course 2026 (24 Hours)",
        durationMin: 300,
        url: "https://www.youtube.com/watch?v=22WQ6u7Cd-c",
        tags: ["javascript", "es6", "async-await", "fetch", "dom"]
      },
      {
        id: "frontend-2026-4",
        title: "JavaScript Logic Building & Advanced Patterns",
        durationMin: 180,
        url: "https://www.youtube.com/watch?v=GmBxMOj2gd4",
        tags: ["javascript", "logic", "patterns", "advanced"]
      },
      {
        id: "frontend-2026-5",
        title: "React 19 Crash Course (90 Minutes)",
        durationMin: 90,
        url: "https://www.youtube.com/watch?v=tqjJrXd27m4",
        tags: ["react", "react19", "hooks", "jsx"]
      },
      {
        id: "frontend-2026-6",
        title: "ReactJS Full Course - Scratch to Advanced",
        durationMin: 240,
        url: "https://www.youtube.com/watch?v=3LRZRSIh_KE",
        tags: ["react", "state-management", "context-api", "custom-hooks"]
      },
      {
        id: "frontend-2026-7",
        title: "Next.js 15 Mastery Course 2026",
        durationMin: 180,
        url: "https://www.youtube.com/watch?v=KAQCHfu_3jw",
        tags: ["nextjs", "app-router", "ssr", "server-actions"]
      },
      {
        id: "frontend-2026-8",
        title: "Next.js 15 Full Course (Build & Deploy) (Hindi)",
        durationMin: 180,
        url: "https://www.youtube.com/watch?v=CMUEmQ6WCGg",
        tags: ["nextjs", "deployment", "hindi", "full-course"]
      },
      {
        id: "frontend-2026-9",
        title: "Git & GitHub in 1 Hour",
        durationMin: 60,
        url: "https://www.youtube.com/watch?v=qIW9Nm3-8rg",
        tags: ["git", "github", "version-control"]
      },
      {
        id: "frontend-2026-10",
        title: "Master Tailwind CSS v4 in React",
        durationMin: 120,
        url: "https://www.youtube.com/watch?v=bnfhmr1v028",
        tags: ["tailwind", "react", "tailwind-v4"]
      },
      {
        id: "frontend-2026-11",
        title: "How Big Companies Hire Frontend Devs in 2026",
        durationMin: 75,
        url: "https://www.youtube.com/watch?v=wHVL07vHlvM",
        tags: ["interview", "career", "hiring", "frontend"]
      }
    ]
  },
  {
    id: "cloud",
    title: "Cloud Engineer",
    role: "Cloud Ops",
    level: "Beginner",
    durationHours: 42,
    description: "Cloud engineering roadmap covering networking, Linux, IaC, containers, DevSecOps, and AI-ready infrastructure.",
    tags: ["cloud", "aws", "azure", "terraform", "kubernetes", "devsecops", "linux", "networking"],
    videos: [
      {
        id: "cloud-2026-1",
        title: "Linux for Cloud Engineers (Full Zero to Hero)",
        durationMin: 240,
        url: "https://www.youtube.com/playlist?list=PLg_uqzorGE2Uc7Mlv5tcpgftIGJVfmx6K",
        tags: ["linux", "bash", "cloud-foundations"]
      },
      {
        id: "cloud-2026-2",
        title: "Networking Concepts for Cloud (2026 Edition)",
        durationMin: 120,
        url: "https://www.youtube.com/watch?v=1JquCES5zuU",
        tags: ["networking", "tcp-ip", "dns", "osi"]
      },
      {
        id: "cloud-2026-3",
        title: "AWS Full Course 2026 (Beginner to Advanced)",
        durationMin: 300,
        url: "https://www.youtube.com/watch?v=41aS-kk1Sjo",
        tags: ["aws", "ec2", "s3", "iam", "vpc"]
      },
      {
        id: "cloud-2026-4",
        title: "Azure Administrator (AZ-104) Full Course",
        durationMin: 300,
        url: "https://www.youtube.com/watch?v=AtQryKJeHf8",
        tags: ["azure", "az-104", "cloud-provider"]
      },
      {
        id: "cloud-2026-5",
        title: "Terraform Zero to Hero (Master IaC)",
        durationMin: 240,
        url: "https://www.youtube.com/playlist?list=PLdpzxOOAlwvI0O4PeKVV1-yJoX2AqIWuf",
        tags: ["terraform", "iac", "hcl", "state-management"]
      },
      {
        id: "cloud-2026-6",
        title: "Terraform + AWS Project: Automate Everything",
        durationMin: 150,
        url: "https://www.youtube.com/watch?v=bBQCYGcjuWE",
        tags: ["terraform", "aws", "automation", "project"]
      },
      {
        id: "cloud-2026-7",
        title: "Kubernetes (K8s) Full Course for Beginners",
        durationMin: 210,
        url: "https://www.youtube.com/watch?v=D-TMKParQ88",
        tags: ["kubernetes", "k8s", "containers", "orchestration"]
      },
      {
        id: "cloud-2026-8",
        title: "Docker & K8s Project Mastery",
        durationMin: 240,
        url: "https://www.youtube.com/playlist?list=PLAdTNzDIZj_9c-o43WJ7yYBa5YEAkWiOv",
        tags: ["docker", "kubernetes", "helm", "project"]
      },
      {
        id: "cloud-2026-9",
        title: "DevOps Roadmap 2026: Complete Learning Path",
        durationMin: 180,
        url: "https://www.youtube.com/watch?v=MD68qernLS0",
        tags: ["devops", "cicd", "devsecops", "github-actions", "jenkins"]
      },
      {
        id: "cloud-2026-10",
        title: "End-to-End Corporate CI/CD Pipeline",
        durationMin: 160,
        url: "https://www.youtube.com/watch?v=7RT4f2Z_Dzs",
        tags: ["cicd", "pipeline", "security", "enterprise"]
      }
    ]
  },
  {
    id: "ai-engineer",
    title: "AI Engineer",
    role: "AI Applications",
    level: "Intermediate",
    durationHours: 34,
    description: "Build production-ready AI apps with LLMs, RAG pipelines, prompt engineering, and model deployment.",
    tags: ["ai", "llm", "rag", "prompting", "deployment", "python"],
    videos: [
      { id: "ai-topic-1", title: "Python for AI Engineers", durationMin: 150, url: "", tags: ["python", "foundations"] },
      { id: "ai-topic-2", title: "Prompt Engineering Fundamentals", durationMin: 120, url: "", tags: ["prompting", "llm"] },
      { id: "ai-topic-3", title: "LLM Application Architecture", durationMin: 140, url: "", tags: ["llm", "architecture"] },
      { id: "ai-topic-4", title: "RAG Systems End-to-End", durationMin: 180, url: "", tags: ["rag", "vector-db"] },
      { id: "ai-topic-5", title: "AI Agent Workflows", durationMin: 130, url: "", tags: ["agents", "automation"] },
      { id: "ai-topic-6", title: "Model Evaluation and Guardrails", durationMin: 110, url: "", tags: ["evaluation", "safety"] },
      { id: "ai-topic-7", title: "Deploying AI APIs", durationMin: 120, url: "", tags: ["deployment", "apis"] },
      { id: "ai-topic-8", title: "AI Capstone Project", durationMin: 180, url: "", tags: ["project", "portfolio"] }
    ]
  },
  {
    id: "ml-engineer",
    title: "ML Engineer",
    role: "Machine Learning",
    level: "Intermediate",
    durationHours: 38,
    description: "Train, evaluate, and deploy ML models with MLOps practices for scalable real-world systems.",
    tags: ["machine-learning", "mlops", "python", "models", "deployment"],
    videos: [
      { id: "ml-topic-1", title: "Math and Statistics Refresher", durationMin: 150, url: "", tags: ["math", "statistics"] },
      { id: "ml-topic-2", title: "Supervised Learning Basics", durationMin: 170, url: "", tags: ["supervised-learning"] },
      { id: "ml-topic-3", title: "Model Evaluation Metrics", durationMin: 120, url: "", tags: ["metrics", "evaluation"] },
      { id: "ml-topic-4", title: "Feature Engineering Techniques", durationMin: 130, url: "", tags: ["features", "preprocessing"] },
      { id: "ml-topic-5", title: "Deep Learning Foundations", durationMin: 180, url: "", tags: ["deep-learning", "neural-networks"] },
      { id: "ml-topic-6", title: "MLOps and Model Versioning", durationMin: 150, url: "", tags: ["mlops", "versioning"] },
      { id: "ml-topic-7", title: "Model Serving and Monitoring", durationMin: 130, url: "", tags: ["serving", "monitoring"] },
      { id: "ml-topic-8", title: "ML Production Project", durationMin: 180, url: "", tags: ["project", "deployment"] }
    ]
  },
  {
    id: "data-analyst",
    title: "Data Analyst",
    role: "Analytics",
    level: "Beginner",
    durationHours: 30,
    description: "Analyze business data using SQL, spreadsheets, Python, and dashboards to drive decision-making.",
    tags: ["sql", "excel", "python", "power-bi", "tableau", "analytics"],
    videos: [
      { id: "da-topic-1", title: "Excel for Data Analysis", durationMin: 140, url: "", tags: ["excel", "spreadsheets"] },
      { id: "da-topic-2", title: "SQL for Analysts", durationMin: 180, url: "", tags: ["sql", "databases"] },
      { id: "da-topic-3", title: "Python Pandas Basics", durationMin: 140, url: "", tags: ["python", "pandas"] },
      { id: "da-topic-4", title: "Data Cleaning and EDA", durationMin: 150, url: "", tags: ["eda", "data-cleaning"] },
      { id: "da-topic-5", title: "Data Visualization Principles", durationMin: 120, url: "", tags: ["visualization"] },
      { id: "da-topic-6", title: "Power BI Dashboard Project", durationMin: 150, url: "", tags: ["power-bi", "dashboard"] },
      { id: "da-topic-7", title: "Tableau for Business Insights", durationMin: 130, url: "", tags: ["tableau", "bi"] },
      { id: "da-topic-8", title: "Analyst Case Study", durationMin: 160, url: "", tags: ["case-study", "portfolio"] }
    ]
  },
  {
    id: "data-scientist",
    title: "Data Scientist",
    role: "Data Science",
    level: "Intermediate",
    durationHours: 40,
    description: "Master statistics, ML, experimentation, and storytelling to solve complex data-driven problems.",
    tags: ["data-science", "python", "statistics", "ml", "experimentation"],
    videos: [
      { id: "ds-topic-1", title: "Python and NumPy Foundations", durationMin: 160, url: "", tags: ["python", "numpy"] },
      { id: "ds-topic-2", title: "Statistics and Probability", durationMin: 180, url: "", tags: ["statistics", "probability"] },
      { id: "ds-topic-3", title: "Exploratory Data Analysis", durationMin: 150, url: "", tags: ["eda", "analysis"] },
      { id: "ds-topic-4", title: "Machine Learning Algorithms", durationMin: 200, url: "", tags: ["ml", "algorithms"] },
      { id: "ds-topic-5", title: "A/B Testing and Experimentation", durationMin: 130, url: "", tags: ["ab-testing", "experiments"] },
      { id: "ds-topic-6", title: "Feature Selection and Tuning", durationMin: 130, url: "", tags: ["feature-selection", "tuning"] },
      { id: "ds-topic-7", title: "Storytelling with Data", durationMin: 110, url: "", tags: ["storytelling", "communication"] },
      { id: "ds-topic-8", title: "Data Science Capstone", durationMin: 190, url: "", tags: ["capstone", "portfolio"] }
    ]
  },
  {
    id: "cybersecurity",
    title: "Cybersecurity Analyst",
    role: "Security",
    level: "Beginner",
    durationHours: 32,
    description: "Learn security fundamentals, threat detection, SOC workflows, and practical defense strategies.",
    tags: ["security", "soc", "network-security", "incident-response", "siem"],
    videos: [
      { id: "sec-topic-1", title: "Cybersecurity Fundamentals", durationMin: 150, url: "", tags: ["fundamentals", "security"] },
      { id: "sec-topic-2", title: "Network Security Basics", durationMin: 140, url: "", tags: ["network-security"] },
      { id: "sec-topic-3", title: "Linux and Security Commands", durationMin: 120, url: "", tags: ["linux", "security"] },
      { id: "sec-topic-4", title: "Threat Detection and SIEM", durationMin: 160, url: "", tags: ["threat-detection", "siem"] },
      { id: "sec-topic-5", title: "Vulnerability Management", durationMin: 120, url: "", tags: ["vulnerabilities"] },
      { id: "sec-topic-6", title: "Incident Response Workflow", durationMin: 130, url: "", tags: ["incident-response"] },
      { id: "sec-topic-7", title: "Cloud Security Basics", durationMin: 130, url: "", tags: ["cloud-security"] },
      { id: "sec-topic-8", title: "Security Operations Project", durationMin: 170, url: "", tags: ["soc", "project"] }
    ]
  },
  {
    id: "mobile-developer",
    title: "Mobile App Developer",
    role: "Mobile",
    level: "Beginner",
    durationHours: 34,
    description: "Build modern Android/iOS apps using cross-platform frameworks, APIs, and deployment best practices.",
    tags: ["mobile", "flutter", "react-native", "android", "ios", "api"],
    videos: [
      { id: "mob-topic-1", title: "Mobile App Architecture Basics", durationMin: 120, url: "", tags: ["architecture", "mobile"] },
      { id: "mob-topic-2", title: "Dart and Flutter Fundamentals", durationMin: 180, url: "", tags: ["dart", "flutter"] },
      { id: "mob-topic-3", title: "React Native Crash Course", durationMin: 160, url: "", tags: ["react-native"] },
      { id: "mob-topic-4", title: "State Management in Mobile Apps", durationMin: 130, url: "", tags: ["state-management"] },
      { id: "mob-topic-5", title: "Working with REST APIs", durationMin: 120, url: "", tags: ["api", "rest"] },
      { id: "mob-topic-6", title: "Authentication and Security", durationMin: 110, url: "", tags: ["auth", "security"] },
      { id: "mob-topic-7", title: "Testing and Debugging Mobile Apps", durationMin: 120, url: "", tags: ["testing", "debugging"] },
      { id: "mob-topic-8", title: "Publish App to Stores", durationMin: 130, url: "", tags: ["deployment", "play-store", "app-store"] }
    ]
  },
  {
    id: "qa-automation",
    title: "QA Automation Engineer",
    role: "Quality Engineering",
    level: "Beginner",
    durationHours: 28,
    description: "Design test strategy and build automated test pipelines for reliable, fast software releases.",
    tags: ["qa", "automation", "selenium", "playwright", "api-testing", "cicd"],
    videos: [
      { id: "qa-topic-1", title: "Testing Fundamentals", durationMin: 120, url: "", tags: ["testing", "qa"] },
      { id: "qa-topic-2", title: "Manual to Automation Mindset", durationMin: 100, url: "", tags: ["automation"] },
      { id: "qa-topic-3", title: "Selenium WebDriver Basics", durationMin: 150, url: "", tags: ["selenium"] },
      { id: "qa-topic-4", title: "Playwright End-to-End Testing", durationMin: 150, url: "", tags: ["playwright", "e2e"] },
      { id: "qa-topic-5", title: "API Testing with Postman", durationMin: 120, url: "", tags: ["api-testing", "postman"] },
      { id: "qa-topic-6", title: "Test Frameworks and Reporting", durationMin: 110, url: "", tags: ["framework", "reports"] },
      { id: "qa-topic-7", title: "CI/CD Integration for Tests", durationMin: 120, url: "", tags: ["cicd", "automation"] },
      { id: "qa-topic-8", title: "Automation Project", durationMin: 150, url: "", tags: ["project", "portfolio"] }
    ]
  }
]

export const PATHS_BY_ID = PATHS.reduce((acc, item) => {
  acc[item.id] = item
  return acc
}, {})
